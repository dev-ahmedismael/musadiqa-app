...
export default function NavbarUser({ user }: Props) {
  ...
  return (
    <ClickAwayListener onClickAway={handleClickAway}>
      <Box
        className="px-5 flex items-center gap-5 border-s border-slate-300 relative cursor-pointer"
      ...
      >
        ...
        {open && (
          <Box className="absolute z-50 bg-white top-full right-0 w-full p-2 border border-slate-300 rounded grid grid-cols-1 gap-2">
            <Box className="flex items-center gap-5 p-2 rounded hover:bg-slate-100 active:bg-slate-200">
              <PortraitIcon />
              <Typography fontSize={"small"}>تعديل الملف الشخصي</Typography>
            </Box>

            <Box
              className="flex items-center gap-5 p-2 rounded hover:bg-slate-100 active:bg-slate-200"
              onClick={logout}
            >
